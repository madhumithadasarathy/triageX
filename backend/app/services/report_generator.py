...
class ReportGenerator:
    ...
    def generate_report(self, triage_data: Dict[str, Any], session_id: str) -> str:
        ...
        for s in symptoms:
            story.append(Paragraph(f"• {s.title()}", self.styles['BodyText']))
        story.append(Spacer(1, 8))

        # Duration & Intensity
        if triage_data.get("duration"):
            story.append(Paragraph(f"<b>Duration:</b> {triage_data['duration']}", self.styles['BodyText']))
        if triage_data.get("intensity"):
            story.append(Paragraph(f"<b>Intensity:</b> {triage_data['intensity']}/10", self.styles['BodyText']))
        if triage_data.get("medical_history"):
            story.append(Paragraph(f"<b>Medical History:</b> {triage_data['medical_history']}", self.styles['BodyText']))
        story.append(Spacer(1, 12))
        ...
        if triggered_rules:
            for rule in triggered_rules:
                ...
                story.append(Paragraph(rule_text, self.styles['BodyText']))
                matched = ", ".join(rule.get("matched_symptoms", []))
                story.append(Paragraph(f"  → Matched: {matched} (Score: {rule.get('severity_contribution', 0)}/100)", self.styles['BodyText']))
        else:
            story.append(Paragraph("No specific clinical rules were triggered. Assessment based on general symptom analysis.", self.styles['BodyText']))
        story.append(Spacer(1, 12))
        ...
        if regions:
            ...
            for region in regions:
                ...
                story.append(Paragraph(f"• <b>{r_name}</b> ({r_severity}): {r_symptoms}", self.styles['BodyText']))
            story.append(Spacer(1, 12))
