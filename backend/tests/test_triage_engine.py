...
def test_triage_engine():
    ...
    # Test LOW severity: mild headache
    result = engine.evaluate(["mild headache"], intensity=2)
    ...
    print(f"✅ Triage: Emergency detection passed")
...
def test_explanation_engine():
    ...
    print("✅ Explanation: Reasoning generation passed")

    # Test key factors
    factors = engine.generate_key_factors(rules, ["chest pain", "difficulty breathing"], 85, 8)
    assert len(factors) > 0
    print(f"✅ Explanation: Key factors passed → {factors}")
    ...
    print("✅ Explanation: Clinical summary passed")
    ...
    print("✅ Explanation: Patient summary passed")
...
def test_visual_mapper():
    ...
    print(f"✅ Visual: Region mapping passed → {region_names}")

    # Test color coding
    for region in regions:
        assert region["color"] in ["#22C55E", "#EAB308", "#EF4444"], f"Invalid color: {region['color']}"
    print("✅ Visual: Color coding passed")
    ...
    print(f"✅ Visual: Condition images passed → {len(images)} images")
